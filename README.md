...
- `AMCP_RAG_EMBEDDING_PROVIDER=openai` enables the OpenAI-compatible provider only when `AMCP_RAG_EGRESS_CONSENT=true` and `OPENAI_API_KEY` are present. `AMCP_RAG_EMBEDDING_MODEL` defaults to `text-embedding-3-small`, `AMCP_RAG_EMBEDDING_DIMENSIONS` is optional, and `AMCP_RAG_EMBEDDING_ENDPOINT` defaults to `https://api.openai.com/v1/embeddings`. AMCP sends only bounded redacted chunks; the key is never stored in the catalog, RAG index, logs or citations.
