...
- w toku: streaming transport/backpressure on top of the bounded event outbox plus broader Agent-side app-server event ingestion; authenticated event replay, post-persistence acknowledgements, central event-ID deduplication, local watcher events, and inventory-only provider defaults are now in place;
