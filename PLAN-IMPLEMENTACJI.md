...
- w toku: bidirectional streaming transport/backpressure on top of the bounded event outbox, incremental local index projections, optional RAG retention/embedding providers, provider parser implementations, packaging/launchd hardening i cross-platform ports; authenticated event replay, bounded `SubscribeEvents` long-poll pages, post-persistence acknowledgements, central event-ID deduplication, local watcher events, inventory-only provider descriptors, Controller-side metadata-only app-server session persistence, Agent-side opt-in Codex app-server thread connector with reconnect/backoff, bounded authenticated `RuntimeListThreads`, scoped human search/runtime activity UI, and lexical RAG retention purge are now in place;
- następne: full runtime IDs/backpressure and true bidirectional streaming, richer provider runtime operations, incremental index projections, optional embedding providers, Claude Code/Antigravity/Kiro parser fixtures and adapters, packaging/launchd hardening i cross-platform ports.
