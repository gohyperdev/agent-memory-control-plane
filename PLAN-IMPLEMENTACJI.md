...
- w toku: bidirectional streaming transport/backpressure on top of the bounded event outbox and broader Agent-side app-server event ingestion; authenticated event replay, bounded `SubscribeEvents` long-poll pages, post-persistence acknowledgements, central event-ID deduplication, local watcher events, inventory-only provider defaults, Controller-side metadata-only app-server session event persistence, and lexical RAG retention purge are now in place;
- następne: full runtime IDs/backpressure, Agent-side app-server thread event connector, incremental local index projections, optional RAG retention/embedding providers, provider fixtures/adapters, packaging/launchd hardening i cross-platform ports.
